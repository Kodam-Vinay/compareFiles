...
async function extractText(buffer, mime) {
  ...
  throw new Error("Only .docx is supported for text extraction.");
}
...
function generateBase64Pdf(text) {
  return new Promise(resolve => {
    ...
    doc.font("Times-Roman").fontSize(12).text(cleanText(text));
    doc.end();
  });
}
...
function generateDiffPDFBuffer(diff) {
  return new Promise((resolve, reject) => {
    ...
    diff.forEach(part => {
      ...
      doc.fillColor(color).text(cleanText(part.value));
    });
    doc.end();
  });
}

function generateDiffPDFBase64(diff) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument();
    const chunks = [];
    doc.on("data", chunk => chunks.push(chunk));
    ...
    diff.forEach(part => {
      const color = part.added ? "green" : part.removed ? "red" : "black";
      const text = cleanText(part.value);
      if (!text) return;
...
app.post("/compare", async (req, res) => {
  try {
    ...
    if (mime1 !== mime2 || !isSupportedMime(mime1)) {
      return res.status(400).json({ status: false, message: "Files must be of the same supported type." });
    }
